...
async function getMockData(companyName) {
  ...
  if (liveNews && liveNews.length > 0) {
    // We successfully fetched live Google News!
    // Let's map them to our sources layout
    sources = liveNews.map(item => ({
      title: item.title,
      url: item.url,
      publisher: item.publisher // This will override domain rendering or show clean source name
    }));
    
    // Dynamically build summary points based on the actual live headlines!
    ...
    // Add a generic final bullet about the financial focus
    summaryBullets += `- 해당 실시간 뉴스 흐름에 기반하여 기관 및 개인 투자자들이 기업의 영업이익 궤적 및 단기 거래량 변동을 집중 모니터링 중입니다.`;
  } else {
    // Fallback static sources if RSS fetch completely fails
    sources = [
...
- 다가오는 거시경제 금리 변동 리스크와 소비 심리 변화에 대응하여, 고부가가치 세그먼트 비중을 늘리고 포트폴리오 정밀 조정을 진행 중입니다.`;
  ...
  // Detect basic stock sentiment based on mock keywords or arbitrary fallback
...
// Serve static assets in production from Vite's 'dist' directory
